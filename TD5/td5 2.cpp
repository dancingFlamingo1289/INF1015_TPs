/**
* Programme qui permet de manipuler une biblitheque, representée par un vecteur, de film et de livres. Il y a aussi la copie de cette bibliothéque 
* et la complexité d'algorithmes qui est prise en compte.
* \file   td4.cpp
* \author Elias Kassas(2381771) et Rym Fortas(2385101)
* \date  2 avril 2025
...
int main(int argc, char* argv[])
{
	...
	// 1.1
	cout << 1.1 << endl;
	forward_list<Item*> lesItemsOriginal;
	for (auto& item : items) {
		lesItemsOriginal.push_front(item.get());
	}
	lesItemsOriginal.reverse(); 
	afficherListeItems(lesItemsOriginal);
	
	// 1.2
	cout << 1.2 << endl;
	forward_list<Item*> lesItemsInverses;
	for (auto it = lesItemsOriginal.begin(); it != lesItemsOriginal.end(); ++it)
		lesItemsInverses.push_front(*it);
	afficherListeItems(lesItemsInverses);

	// 1.3
	cout << 1.3 << endl;
	forward_list<Item*> lesItemsCopie;
	auto it = lesItemsCopie.before_begin();
	for (auto& item : lesItemsOriginal) {
		it = lesItemsCopie.insert_after(it, item);  // Copie profonde de l'objet
	}
	afficherListeItems(lesItemsCopie);

	// 1.4 - compl. : O(n)
	cout << 1.4 << endl;
	vector<Item*> itemsInverse;
	for (auto& item : lesItemsOriginal) {
		itemsInverse.push_back(item);  // Copie profonde de l'objet
	}
	afficherListeItems(itemsInverse);
	...
	//	// Création d'acteurs
	//	auto acteur1 = make_shared<Acteur>();
	//	acteur1->nom = "Sigourney Weaver";
	//	acteur1->anneeNaissance = 1949;
	//	acteur1->sexe = 'F';

	//	auto acteur2 = make_shared<Acteur>();
	//	acteur2->nom = "Tom Skerritt";
	//	acteur2->anneeNaissance = 1933;
	//	acteur2->sexe = 'M';

	//	film.acteurs = ListeActeurs(4);
	//	// Ajout des acteurs au film
	//	film.acteurs.ajouter(acteur1);
	//	film.acteurs.ajouter(acteur2);
	...
	cout << ligneDeSeparation << 2.1 << endl;
	set<Item*, function<bool(Item*, Item*)>> itemsEnOrdreAlpha(
		[](Item* a, Item* b) {
			return a->titre < b->titre;  // Comparaison des noms
		}
	);
	for (auto& item : lesItemsOriginal) {
		itemsEnOrdreAlpha.insert(item);
	}
	afficherListeItems(itemsEnOrdreAlpha);

	// 2.2
	
	cout << ligneDeSeparation << endl;
	cout << "2.2 Recherche de 'The Hobbit' :\n";

	std::unordered_map<string, shared_ptr<Item>> conteneur;

	for (auto& item : items) {
		conteneur[item->titre] = item;
	}

	string titreRecherche = "The Hobbit";
	auto search = conteneur.find(titreRecherche);

	if (search != conteneur.end()) {
		cout << "Trouvé : " << search->first << " → " << *search->second << '\n';
	} else {
		cout << "Non trouvé ";
	}
	...
	cout << ligneDeSeparation << endl;
	cout << "3.1 :\n";
	vector<Item*> mesItems;
	copy_if(lesItemsOriginal.begin(), lesItemsOriginal.end(), std::back_inserter(mesItems), [](Item* item) {
		return dynamic_cast<Film*>(item) != nullptr;
	});
	afficherListeItems(mesItems);

	// 3.2
	int totalRecettes = accumulate(mesItems.begin(), mesItems.end(), 0, [](int sum, Item* item) {
		return sum + dynamic_cast<Film*>(item)->recette;
	});
	cout <<"la somme des recettes des films trouvés est :"<< totalRecettes;
}
