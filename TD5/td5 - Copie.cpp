...
int main(int argc, char* argv[])
{
	...
	// 1.1
	cout << 1.1 << endl;
	forward_list<shared_ptr<Item>> lesItemsOriginal;
	for (int i = items.size() - 1 ; i >= 0 ; i--)
		lesItemsOriginal.push_front(make_shared<Item>(*items[i]));
	afficherListeItems(lesItemsOriginal);
	
	// 1.2
	cout << 1.2 << endl;
	forward_list<shared_ptr<Item>> lesItemsInverses;
	for (auto it = lesItemsOriginal.begin(); it != lesItemsOriginal.end(); ++it)
		lesItemsInverses.push_front(make_shared<Item>(**it));
	afficherListeItems(lesItemsInverses);

	// 1.3
	cout << 1.3 << endl;
	forward_list<shared_ptr<Item>> lesItemsCopie;
	auto it = lesItemsCopie.before_begin();
	for (auto& item : lesItemsOriginal) {
		it = lesItemsCopie.insert_after(it, make_shared<Item>(*item));  // Copie profonde de l'objet
	}
	afficherListeItems(lesItemsCopie);

	// 1.4 - compl. : O(n)
	cout << 1.4 << endl;
	vector<shared_ptr<Item>> itemsInverse;
	for (auto& item : lesItemsOriginal) {
		itemsInverse.push_back(make_shared<Item>(*item));  // Copie profonde de l'objet
	}
	...
	set<shared_ptr<Item>, function<bool(shared_ptr<Item>, shared_ptr<Item>)>> itemsEnOrdreAlpha(
		[](shared_ptr<Item> a, shared_ptr<Item> b) {
	...
	// 2.2
	cout << ligneDeSeparation << 2.2 << endl;
	...
	// Partie #3 TD5
	// 3.1
	//vector<shared_ptrItem> mesItems;
	...
	// 3.2
	//int totalRecettes = accumulate(mesItems.begin(), mesItems.end(), 0, [](int sum, Film* film) {
	//	return sum + film->recette;
	//});
	//cout << totalRecettes;
}
