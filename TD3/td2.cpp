...
{	
	if (film == nullptr) {
		//std::cerr << "Erreur : le pointeur film est nullptr" << std::endl;
		return;
	}
...
//span<shared_ptr<T>> ListeActeurs::spanListe(const ListeActeurs& liste) { return span<shared_ptr<T>>(liste.getElements().get(), liste.getNElements()); }
...
)
{
	...
	//[
	Film* filmp = new Film(move(film)); //NOTE: On aurait normalement fait le "new" au début de la fonction pour directement mettre les informations au bon endroit; on le fait ici pour que le code ci-dessus puisse être directement donné aux étudiants sans qu'ils aient le "new" déjà écrit.
	cout << "Création Film " << filmp->titre << endl;
	filmp->acteurs = ListeActeurs(nElements);
	...
	*/
	...
	for (shared_ptr<Acteur>& acteur : filmp->acteurs.spanListe()) {
		acteur = 
		//]
		lireActeur(fichier//[
		, listeFilms//]
...
ListeFilms::ListeFilms(const string& nomFichier) : possedeLesFilms_(true), nElements(0), elements(nullptr)
{
	ifstream fichier(nomFichier, ios::binary);
	fichier.exceptions(ios::failbit);

	int nElements = lireUint16(fichier);
	capacite = nElements + 1;
	
	elements = new Film * [capacite]; // Allocation du tableau de pointeurs
...
void detruireFilm(Film* film)
{
	...
	//film->acteurs.getElements().reset();
...
ListeFilms::~ListeFilms()
{
	...
	delete[] elements ;
	possedeLesFilms_ = false ;
}
//]

void afficherActeur(ostream& os, const Acteur& acteur)
{
...
int main()
{
	...
	//]
	cout << ligneDeSeparation << "Le film dont la recette est egale à 955 M$ est : " << endl;
	cout << *listeFilms.chercherFilm([](Film* leFilm) -> bool {
		return leFilm->recette == 955;}) << endl;
...
//]
	...
	// vérifier avec string 
	Liste<string> listeTextes;
	listeTextes.setCapacite(2);
	listeTextes.setElements(make_unique<shared_ptr<string>[]>(2));
	listeTextes.setNElements(2);
	listeTextes[0] = make_shared<string>("Rym");
	listeTextes[1] = make_shared<string>("Elias");
	Liste<string> listeTextes2 = listeTextes;

	cout << ligneDeSeparation << "la listeTextes est : " << endl;
	for (int i = 0; i < listeTextes.getNElements(); i++) {
		cout << *(listeTextes[i]) << endl;
	}

	cout << "\033[32m────────────────────────────────────────\033[0m\n" << "la listeTextes2 est : " << endl;
	for (int i = 0; i < listeTextes2.getNElements(); i++) {
		cout << *(listeTextes2[i]) << endl;
	}

	// modification :
	listeTextes[0] = make_shared<string>("Hola");
	*(listeTextes[1]) = "hi";

	cout << ligneDeSeparation << "la nouvelle listeTextes est : " << endl;
	for (int i = 0; i < listeTextes.getNElements(); i++) {
		cout << *(listeTextes[i]) << endl;
	}

	cout << "\033[32m────────────────────────────────────────\033[0m\n" << "la listeTextes2 apres la modification est : " << endl;
	for (int i = 0; i < listeTextes2.getNElements(); i++) {
		cout << *(listeTextes2[i]) << endl;
	}
...
//		std::cerr << "Erreur d'allocation de mémoire : " << e.what() << std::endl;
//		std::cerr << "Erreur à la ligne : " << __LINE__ << std::endl;
//	}
//}
