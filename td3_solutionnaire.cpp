...
Film* lireFilm(istream& fichier, ListeFilms& listeFilms)
{
	...
	auto nacteurs_ = lireUint8 (fichier);
	film->acteurs_ = ListeActeurs(nacteurs_);  // On n'a pas fait de méthode pour changer la taille d'allocation, seulement un constructeur qui prend la capacité.  Pour que cette affectation fonctionne, il faut s'assurer qu'on a un operator= de move pour Listeacteurs_.
	cout << "Création Film " << film->getTitre() << endl;

	for ([[maybe_unused]] auto i : range(nacteurs_)) {  // On peut aussi mettre nElements avant et faire un span, comme on le faisait au TD précédent.
...
// Fonction pour afficher un film avec tous ces acteurs_ (en utilisant la fonction afficherActeur ci-dessus).
//[
/*ostream& operator<< (ostream& os, const Film& film)
{
	os << "Titre: " << film.getTitre() << endl;
	os << "  Réalisateur: " << film.getRealisateur() << "  Année :" << film.getAnneeSortie() << endl;
	os << "  Recette: " << film.getRecette() << "M$" << endl;
...
/*ostream& operator<< (ostream& os, const Livre& livre)
{
	...
	os << "  Auteur: " << livre.getAuteur() << "  Année : " << livre.getAnneeSortie() << endl;
	os << "  Copies vendues: " << livre.getNCopiesVendues() << " millions" << endl;
	os << "  Nombre de pages: " << livre.getNPages() << endl;
	return os;
}*/
...
void afficherBibliotheque(ostream& os, vector<shared_ptr<Item>>& bibliotheque) {
...
int main()
{
	...
	// etape #2 TD4
	cout << ligneDeSeparation << "Création de la bibliothèque..." << endl;
	...
	while (getline(fichier, ligne)) {
		shared_ptr<Livre> livre = make_shared<Livre>(Livre(ligne));
		bibliotheque.push_back(livre);
	}

	// ajout des films a la bibliotheque
	for (Film* film : listeFilms.enSpan())
		bibliotheque.push_back(make_shared<Film>(*film)) ;

	afficherBibliotheque(cout, bibliotheque) ;
